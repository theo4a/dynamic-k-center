...
from utils import boxplot, check_radius, euclidean_distance, hsva_to_rgba, simulate_streaming, write_json, generate_clustered_points


def experiment_cluster_separation() -> None:
    """
    ...
    for sigma_cluster in cluster_sds:

        results[str(sigma_cluster)] = {
            "sec_GO": [],
            "R_GO": [],
        ...
        for i in range(sample_size):
            ...
            results[str(sigma_cluster)]["sec_GO"].append(go_end - go_start)
            results[str(sigma_cluster)]["R_GO"].append(go_solution["radius"])
            ...
            results[str(sigma_cluster)]["sec_DA"].append(da_end - da_start)
            results[str(sigma_cluster)]["r_prime_DA"].append(da_solution["radius"])
            results[str(sigma_cluster)]["R_DA"].append(da_c_r)
            ...
            results[str(sigma_cluster)]["sec_RDA"].append(rda_end - rda_start)
            results[str(sigma_cluster)]["r_prime_RDA"].append(rda_solution["radius"])
            results[str(sigma_cluster)]["R_RDA"].append(rda_c_r)
            ...
            results[str(sigma_cluster)]["sec_PSA"].append(psa_end - psa_start)
            results[str(sigma_cluster)]["r_prime_PSA"].append(psa_solution["radius"])
            results[str(sigma_cluster)]["R_PSA"].append(psa_c_r)

    stats = []
    for sigma_cluster, value in results.items():
    ...
    write_json("experiment_cluster_separation", data)


def plot_experiment_cluster_separation() -> None:
    
    file_path = os.path.join(os.path.dirname(__file__), "..", "results", "data", "experiment_cluster_separation.json")
    ...
    fig, ax = boxplot(
        data=data["results"],
        x_label=r"$\sigma_{cluster}$",
        y_label="",
        algorithms=algorithms,
        colors=colors
    )

    plot_file_path = os.path.join(os.path.dirname(__file__), "..", "results", "plots", "plot_experiment_cluster_separation.jpg")
