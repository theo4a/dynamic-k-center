...
def experiment_runtime_k() -> None:
    ...
    stats = {
        ...
        "mean(sec_PSA/sec_DA)": statistics.mean([psa/da for psa, da in zip(results["sec_PSA"], results["sec_DA"])])
    }
    ...
    # Save results
    write_json("experiment_runtime_k", data)


def plot_experiment_runtime_k() -> None:
    
    file_path = os.path.join(os.path.dirname(__file__), "..", "results", "data", "experiment_runtime_k.json")
    ...
    plot_file_path = os.path.join(os.path.dirname(__file__), "..", "results", "plots", "plot_experiment_runtime_k.jpg")
