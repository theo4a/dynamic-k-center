...
from utils import chebyshev_distance, check_radius, euclidean_distance, hsva_to_rgba, manhattan_distance, simulate_streaming, write_json, boxplot, generate_clustered_points


def experiment_metric() -> None:
    """
    ...
    for metric_name, d in metrics.items():
        ...
        for i in range(sample_size):
            ...
            results[metric_name]["r_prime_DA"].append(da_solution["radius"])
            ...
            results[metric_name]["r_prime_RDA"].append(rda_solution["radius"])
            ...
            results[metric_name]["r_prime_PSA"].append(psa_solution["radius"])
    ...
    # Save results
    write_json("experiment_metric", data)


def plot_experiment_metric() -> None:
    
    file_path = os.path.join(os.path.dirname(__file__), "..", "results", "data", "experiment_metric.json")
    ...
    plot_file_path = os.path.join(os.path.dirname(__file__), "..", "results", "plots", "plot_experiment_metric.jpg")
